                ...
                <div class="space-y-4">
                    ...
                    <div class="flex justify-start">
                        <flux:button variant="danger" icon="shield-exclamation" icon:variant="outline" wire:click="disable"
                            wire:target="disable" wire:loading.attr="disabled" wire:loading.class="opacity-75 cursor-wait">
                            {{ __('Disable 2FA') }}
                        </flux:button>
                    </div>
                </div>
            @else
                <div class="space-y-4">
                    ...
                    <flux:button variant="primary" icon="shield-check" icon:variant="outline" wire:click="enable"
                        wire:target="enable" wire:loading.attr="disabled" wire:loading.class="opacity-75 cursor-wait">
                        {{ __('Enable 2FA') }}
                    </flux:button>
                </div>
            @endif
                ...
                <div class="space-y-6">
                    ...
                    <div class="flex items-center space-x-3">
                        <flux:button variant="outline" class="flex-1" wire:click="resetVerification"
                            wire:target="resetVerification" wire:loading.attr="disabled"
                            wire:loading.class="opacity-75 cursor-wait">
                            {{ __('Back') }}
                        </flux:button>

                        <flux:button variant="primary" class="flex-1" wire:click="confirmTwoFactor"
                            wire:target="confirmTwoFactor" wire:loading.attr="disabled"
                            wire:loading.class="opacity-75 cursor-wait" x-bind:disabled="$wire.code.length < 6">
                            {{ __('Confirm') }}
                        </flux:button>
                    </div>
                </div>
            @else
                ...
                <div>
                    <flux:button :disabled="$errors->has('setupData')" variant="primary" class="w-full"
                        wire:click="showVerificationIfNecessary" wire:target="showVerificationIfNecessary"
