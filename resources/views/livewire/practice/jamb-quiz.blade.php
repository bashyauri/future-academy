    ...
    <div wire:key="jamb-quiz-mode" class="space-y-4 md:space-y-6 p-4 md:p-6">
        ...
        <div class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 md:p-6 bg-white dark:bg-neutral-800">
            <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                <div class="flex-1">
                    <flux:heading size="lg" level="1" class="mb-1">JAMB {{ $year }} Practice Test</flux:heading>
        ...
        <div class="overflow-x-auto">
            <div class="flex gap-2">
                @foreach($subjectsData as $index => $subject)
                    ...
                    <button
                        wire:key="tab-{{ $subject->id }}"
                        wire:click="switchSubject({{ $index }})"
                        wire:loading.attr="disabled"
                        wire:target="switchSubject"
                        class="flex-shrink-0 px-4 py-3 rounded-lg whitespace-nowrap transition-all {{ $currentSubjectIndex == $index ? 'bg-green-500 text-white shadow-md' : 'bg-white dark:bg-neutral-800 text-neutral-700 dark:text-neutral-300 hover:bg-neutral-50 dark:hover:bg-neutral-700 border border-neutral-200 dark:border-neutral-700' }}">
                        <div class="font-semibold text-sm">{{ $subject->name }}</div>
                        <div class="text-xs mt-1 {{ $currentSubjectIndex == $index ? 'text-green-100' : 'text-neutral-500 dark:text-neutral-400' }}">
                            <span class="font-medium">{{ $answered }}</span>/{{ $questionsPerSubject }}
                        </div>
                    </button>
                @endforeach
            </div>
        ...
        <div x-show="showProgressGrid" x-transition x-cloak class="lg:hidden mb-6 rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 p-4">
            <div class="space-y-4">
                @foreach($subjectsData as $subjectIndex => $subject)
                    @php $answered = count(array_filter($userAnswers[$subject->id])); @endphp
                    <div wire:key="mobile-subject-{{ $subject->id }}">
                        <div class="flex items-center justify-between mb-2">
                            ...
                            <flux:text class="text-xs text-neutral-500 dark:text-neutral-400">{{ $answered }}/{{ $questionsPerSubject }}</flux:text>
                        </div>
                        <div class="grid grid-cols-10 gap-1">
                            @for($i = 0; $i < $questionsPerSubject; $i++)
                                @php
                                    $isAnswered = ($userAnswers[$subject->id][$i] ?? null) !== null;
                                    $isCurrent = $currentSubjectIndex == $subjectIndex && $currentQuestionIndex == $i;
                                @endphp
                                <button
                                    wire:key="mobile-q-{{ $subjectIndex }}-{{ $i }}"
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
            ...
            <div class="lg:col-span-2 space-y-4 md:space-y-6">
                ...
                <!-- Question Header -->
                <div wire:key="question-header-{{ $this->getCurrentSubjectId() }}-{{ $currentQuestionIndex }}" class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 md:p-6 bg-white dark:bg-neutral-800">
                    <div class="flex items-center justify-between mb-4">
                        <flux:heading size="sm" class="text-base md:text-lg font-semibold mb-0">
                            Question {{ $currentQuestionIndex + 1 }} of {{ $questionsPerSubject }}
                        </flux:heading>
                        <flux:text class="text-xs md:text-sm text-neutral-600 dark:text-neutral-400 font-medium">
                            {{ round(($currentQuestionIndex + 1) / max(1, $questionsPerSubject) * 100) }}%
                        </flux:text>
                    </div>
                    <div class="w-full bg-neutral-200 dark:bg-neutral-700 rounded-full h-2.5">
                        <div class="bg-blue-600 dark:bg-blue-500 h-2.5 rounded-full transition-all" style="width: {{ ($currentQuestionIndex + 1) / max(1, $questionsPerSubject) * 100 }}%"></div>
                    </div>
                </div>

                <!-- Question Text -->
                <div wire:key="question-text-{{ $this->getCurrentSubjectId() }}-{{ $currentQuestionIndex }}" class="rounded-xl border border-neutral-200 dark:border-neutral-700 p-4 md:p-8 bg-white dark:bg-neutral-800">
                    <flux:heading size="lg" class="text-lg md:text-xl leading-relaxed mb-4">{{ $question->question_text }}</flux:heading>
                </div>
                    ...
                    @foreach($question->options as $option)
                        @php
                            ...
                            if ($hasAnswered) {
                                ...
                                $borderClass = $isSelected ? 'border-blue-500 bg-blue-50 dark:bg-blue-900/20' : 'border-neutral-200 dark:border-neutral-700 hover:border-blue-400 dark:hover:border-blue-600';
                            ...
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-shrink-0 mt-0.5">
                                    @if($hasAnswered)
                                        ...
                                        @endif
                                    @else
                                        <div class="w-6 h-6 rounded-full border-2 flex items-center justify-center {{ $isSelected ? 'border-blue-500 bg-blue-500' : 'border-neutral-300 dark:border-neutral-600' }}">
                                            @if($isSelected)
                ...
                <div wire:key="explain-{{ $question->id }}" class="rounded-lg border border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/20 p-4 animate-fade-in">
            ...
            <div class="mb-6 pb-6 border-b-2 border-blue-200 dark:border-blue-800">
                <div class="flex items-center justify-between mb-3">
                    ...
                    <flux:badge color="blue" class="text-xs">{{ $currentAnswered }}/{{ $questionsPerSubject }}</flux:badge>
                </div>
                ...
                <div>
                    ...
                    @foreach($subjectsData as $subjectIndex => $subject)
                        @if($subjectIndex != $currentSubjectIndex)
                            <div wire:key="other-subject-{{ $subject->id }}" class="mb-4 pb-3 border-b border-gray-200 dark:border-gray-700">
                                <div class="flex items-center justify-between mb-2">
                                    <button
                                        wire:click="switchSubject({{ $subjectIndex }})"
                                    ...
                                    @php $answered = count(array_filter($userAnswers[$subject->id])); @endphp
                                    <flux:text class="text-xs text-gray-500 dark:text-gray-400">{{ $answered }}/{{ $questionsPerSubject }}</flux:text>
                                </div>
                                <div class="grid grid-cols-8 gap-1">
                                    @for($i = 0; $i < $questionsPerSubject; $i++)
                                        ...
                                        @endphp
                                        <button
                                            wire:key="other-q-{{ $subjectIndex }}-{{ $i }}"
                                            wire:click="jumpToQuestion({{ $subjectIndex }}, {{ $i }})"
                                            wire:loading.attr="disabled"
                                            wire:target="jumpToQuestion"
                                            title="Q{{ $i + 1 }}"
