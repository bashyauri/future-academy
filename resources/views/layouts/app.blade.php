...
<body class="min-h-screen bg-white dark:bg-zinc-800">
    ...
    <flux:sidebar sticky stashable class="border-e border-zinc-200 bg-zinc-50 dark:border-zinc-700 dark:bg-zinc-900">
        ...
        <flux:navlist variant="outline">
            <flux:navlist.group :heading="__('Platform')" class="grid">
                ...
                <flux:navlist.item icon="book-open" :href="route('lessons.subjects')"
                    :current="request()->routeIs('lessons.*')" wire:navigate>
                    {{ __('Lessons') }}
                </flux:navlist.item>
                <flux:navlist.item icon="academic-cap" :href="route('quizzes.index')"
                    :current="request()->routeIs('quizzes.*', 'quiz.*')" wire:navigate>
                    {{ __('Quizzes') }}
                </flux:navlist.item>
                <flux:navlist.item icon="play-circle" :href="route('practice.home')"
                    :current="request()->routeIs('practice.*')" wire:navigate>
                    {{ __('Practice') }}
                </flux:navlist.item>
        ...
        <flux:dropdown class="hidden lg:block" position="bottom" align="start">
            <flux:profile :name="auth()->user()?->name ?? __('Guest')" :initials="auth()->user()?->initials() ?? 'G'"
